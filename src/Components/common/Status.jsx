...
const Status = ({ status, themeColor, size, className }) => {
  let bg;
  switch (status) {
    case "online":
      bg = "bg-[#2ea153]";
  ...
  return (
    <div
      className={`${className} rounded-full ${size ? `w-[${size}px] h-[${size}px` : 'w-[14px] h-[14px]' } absolute overflow-hidden translate-y-0.5 p-[2px] ${
        themeColor ? themeColor : "bg-[#2d2f33]"
      }`}
    >
      <div
